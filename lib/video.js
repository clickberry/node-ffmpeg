...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	 ...
	 */
	var execCommand = function (callback, folder, inputLess) {
		...
		var finalCommands = [inputLess ? 'ffmpeg' : 'ffmpeg -i']
			.concat(inputLess ? ' ' : inputs.join(' -i '))
	...
	/**
	 * Splits video into set of segments (using keyframes) without transcoding.
	 */
	this.fnExtractSegments = function (destionationFilePattern, callback) {
		var destinationFolder = path.dirname(destionationFilePattern);
		
		resetCommands(this);
		
		// Adding commands to the list
		this.addCommand('-acodec', 'copy');
		this.addCommand('-vcodec', 'copy');
		this.addCommand('-f', 'segment');
		this.addCommand('-reset_timestamps', '1');
		this.addCommand('-map', '0');
		this.addCommand('-an'); // do not copy audio
		
		// Create the directory to save the segments
		utils.mkdir(destinationFolder, 0777);

		// Add destination file path to the command list
		setOutput(destionationFilePattern);
	...
	/**
	 * Concatenates video segments of the same format.
	 *
	 */
	this.fnConcatSegments = function (segments, callback) {
		// Building the final path
		var segmentPath = segments[0];
		var destinationDirName		= path.dirname(segmentPath)
			, destinationFileBase		= /^[^_]+/.exec(path.basename(segmentPath, path.extname(segmentPath)))[0]
		  , destinationFileName  	= destinationFileBase + '_output' + path.extname(segmentPath)
		  , finalPath				= path.join(destinationDirName, destinationFileName);

		// building file list
		var fileListPath = path.join(destinationDirName, destinationFileBase + '_files.txt');
		...
		segments.forEach(function (segment) {
			...
			fileListContents += 'file \'' + fileName + '\'\n';
		});

		var self = this;
		fs.writeFile(fileListPath, fileListContents, function(err) {
			if(err) return callback(err);

			self.addCommand('-f', 'concat');
			self.addCommand('-i', fileListPath);
			self.addCommand('-c', 'copy');
			...
			// Executing the commands list
			execCommand.call(self, function (err) {
				if (err) return callback(err);

				// delete file list file
				fs.unlink(fileListPath, function (err) {
					if (err) return callback(err);
	...
	/**
	 * Encodes source to MP4 format.
	 */
	this.fnEncodeToMp4 = function (size, callback) {
		// Building the final path
		var destinationDirName		= path.dirname(this.file_path)
		  , destinationFileNameWE	= path.basename(this.file_path, path.extname(this.file_path)) + '_' + size + '.mp4'
		...
		// Adding commands to the list
		this.addCommand('-vf', '"scale=trunc(oh*a/2)*2:min(' + size + '\\,iw)"');

		// setting output
		setOutput(finalPath);
		
		// Executing the commands list
		return execCommand.call(this, function (err) {
			if (err) return callback(err);
			callback(null, finalPath);
		});
	}
	...
	/**
	 * Encodes source to WebM format.
	 */
	this.fnEncodeToWebM = function (size, callback) {
		// Building the final path
		var destinationDirName		= path.dirname(this.file_path)
		  , destinationFileNameWE	= path.basename(this.file_path, path.extname(this.file_path)) + '_' + size + '.webm'
		  , finalPath				= path.join(destinationDirName, destinationFileNameWE);
	...
	/**
	 * Encodes source to Gif format.
	 */
	this.fnEncodeToGif = function (size, duration, callback) {
		...
		// Building the final path
		var destinationDirName		= path.dirname(this.file_path)
		  , destinationFileNameWE	= path.basename(this.file_path, path.extname(this.file_path)) + '_' + size + '.gif'
		  , finalPath				= path.join(destinationDirName, destinationFileNameWE);
